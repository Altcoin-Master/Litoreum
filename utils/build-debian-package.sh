...
PACKAGE_VERSION=$($SRC_PATH/src/litoreumd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/litoreumd $DEB_BIN
cp $SRC_PATH/src/litoreum-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/litoreum-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/litoreumd.1 $DEB_MAN
cp $SRC_DOC/man/litoreum-cli.1 $DEB_MAN
cp $SRC_DOC/man/litoreum-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/litoreumd.bash-completion $DEB_CMP/litoreumd
cp $SRC_PATH/contrib/litoreum-cli.bash-completion $DEB_CMP/litoreum-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/litoreumd.1
gzip --best -n $DEB_MAN/litoreum-cli.1
gzip --best -n $DEB_MAN/litoreum-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/litoreumd $DEB_BIN/litoreum-cli
