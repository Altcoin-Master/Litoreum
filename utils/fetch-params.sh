...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/LitoreumParams"
else
    PARAMS_DIR="$HOME/.litoreum-params"
fi
...
DOWNLOAD_URL="https://github.com/Altcoin-Master/litoreum/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with LTRM_DISABLE_SOMETHING=1
LTRM_DISABLE_WGET="${LTRM_DISABLE_WGET:-}"
#LTRM_DISABLE_IPFS="${LTRM_DISABLE_IPFS:-}"
LTRM_DISABLE_CURL="${LTRM_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$LTRM_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$LTRM_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$LTRM_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Litoreum parameters!
...
main() {
    ...
    cat <<EOF
LTRM - fetch-params.sh

This script will fetch the Litoreum SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Litoreum SNARK parameters. Note that it is
