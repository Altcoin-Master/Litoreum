...
const char * const BITCOIN_CONF_FILENAME = "litoreum.conf";
const char * const BITCOIN_PID_FILENAME = "litoreumd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "litoreumd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LitoreumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LitoreumCore
    // Mac: ~/Library/Application Support/LitoreumCore
    // Unix: ~/.litoreumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LitoreumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LitoreumCore";
#else
    // Unix
    return pathRet / ".litoreumcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty litoreum.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Litoreum Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Litoreum Core developers";
    }
