...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 LITOREUM irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on litoreumcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in litoreum.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("litoreum.conf", statusObj);
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in litoreum.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Litoreum features */
    { "litoreum",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "litoreum",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "litoreum",               "gobject",                &gobject,                {} },
    { "litoreum",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
