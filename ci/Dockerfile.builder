...
# litoreum_hash
RUN git clone https://github.com/Altcoin-Master/litoreum_hash
RUN cd litoreum_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} litoreum
RUN useradd -u ${USER_ID} -g litoreum -s /bin/bash -m -d /litoreum litoreum
...
RUN mkdir /litoreum-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /litoreum-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /litoreum-src
